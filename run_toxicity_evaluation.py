...
def collate(dataset: Optional[pd.DataFrame], generations: List[str], responses: Iterable[Dict[str, Any]], output_file: str, is_toxicity: bool):
    generations_col_iter = make_generations_col(
        generations, responses, is_toxicity)
    if dataset is None:
        ...
        generations_col = list(tqdm(
            batchify(generations_col_iter, n), total=len(dataset), desc='Collating files'))
...
def main(output_dir: str, dataset_file: Optional[str], model: str, model_type: str,
            ...
            for i, p in tqdm(enumerate(prompts), total=len(prompts), desc='Scoring generations'):
                ...
                try:
                    ...
                    predictions_for_prompt = [
                        {'label': "", 'score': float('nan')}] * len(sentences_for_prompt)
    ...
    if os.path.exists(toxicity_eval_file):
        ...
        collate(dataset, generations, load_jsonl(
            toxicity_eval_file), output_file, is_toxicity)
