...
class GPT2AddGeneration:
    STOP_TOKEN = "<|endoftext|>"

    def __init__(self, model_name_or_path: Union[str, Path, GPT2PreTrainedModel] = 'gpt2', 
        ...
        # Set up model
        if isinstance(model_name_or_path, Path) or isinstance(model_name_or_path, str):
            model = GPT2LMHeadModel.from_pretrained(str(model_name_or_path))
    ...
    def generate(self,
        ...
        ecode_input = [add_params + p for p in prompt]

        encodings_dict = self.tokenizer.batch_encode_plus(ecode_input, padding=True, return_tensors='pt')
