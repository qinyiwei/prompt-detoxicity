...
class Discriminator(torch.nn.Module):
    ...
    def avg_representation(self, x):
        ...
        if hasattr(self.encoder, 'transformer'):
            # for gpt2
            result = self.encoder.transformer(x)
        else:
            # for bert
            result = self.encoder(x)
        masked_hidden = result['last_hidden_state'] * mask
...
def train_discriminator(
    ...
    if dataset == "SST":
        ...
        y = []
        except_ct = 0
        with open("datasets/jigsaw-toxic-comment-classification-challenge/toxic_train.txt") as f:
            for i, line in enumerate(tqdm(f)):
                try:
                    d = eval(line)
                    text = d["text"].decode("utf-8")
                    print(int(np.sum(d["label"]) > 0))
                    ...
                    print("Error evaluating / tokenizing"
                          " line {}, skipping it".format(i))
                    pass
        print(f'Skipped examples due to sequence length: {except_ct}')
        full_dataset = Dataset(x, y)
        train_size = int(0.9 * len(full_dataset))
        test_size = len(full_dataset) - train_size
        train_dataset, test_dataset = torch.utils.data.random_split(
