...
def main(rates: int):
    ...
    generations_file = output_dir / 'negativity_gte99.txt'
    ...
    for rate in rates:
        output_file = output_dir / \
            'negativity_gte99_most_neg_{}.txt'.format(rate)
        with open(output_file, 'w') as fo:
            for gen in sorted_gens[:int(len(generations)*rate/100)]:
                fo.write(gen+'\n')

        print("save {} percent negative texts to {}".format(rate, output_file))
